...
Fourteen figures, each as SVG and PNG. Ten are drawn from measured data; four
...
`plot.py` covers EXP-001, `plot_exp002.py` covers EXP-002 and
`plot_exp003.py` covers EXP-003 and the foreground-QoS thread. All three read
only from `data/` and need nothing but matplotlib.
...
## EXP-003 — background canonical-state recovery

Regenerated by `plot_exp003.py` from `data/exp-003/` and
`data/recovery-foreground-qos/`.

- **fig12-canonical-debt** — the logical prompt, the reusable canonical prefix
  and the gap between them, per turn, for the sparse arm and the recovery arm.
- **fig13-session-result** — per-turn TTFT and cumulative foreground time for
  the same two arms. One controlled workload; both arms stay on the sparse
  route for every turn.
- **fig14-foreground-qos** — what a foreground request waits behind, against
  the recovery execution slice. Observed client latency and the trace-derived
  execution bound are plotted as separate series on purpose: they answer
  different questions, and the smallest slice does not win both.

Evidence level 3 for fig12 and fig13 — synthetic interactive workload, one run
per arm. Evidence level 3 for the observed series in fig14 and level 5 for the
trace-derived bound, which is the runtime's own per-slice timing. Sources:
`data/exp-003/spec-exit-always-sparse-{turns,summary}.csv` and
`data/recovery-foreground-qos/{collision-summary,slice-timing}.csv`.

No figure is drawn from
`data/recovery-foreground-qos/recovery-state-retirement.csv`. Its result is a
six-row table and a negative control, and a plot of it would imply a trend
across a variable that has three points and no curvature.
